...
class TeamManager(
    ...
    override fun getPlayerTeam(player: ServerPlayerEntity): TeamHandle? {
        return player.server.scoreboard.getScoreHolderTeam(player.nameForScoreboard)
            ?.let { TeamHandle(it) }
    }
    ...
    override fun setPlayerTeam(player: ServerPlayerEntity, team: ITeamHandle?) {
        ...
        if (team != null) {
            player.server.scoreboard.addScoreHolderToTeam(player.nameForScoreboard, team.team)
        } else {
            // leave the player's current team (set to null)
            val currentTeam = getPlayerTeam(player) ?: return
            player.server.scoreboard.removeScoreHolderFromTeam(player.nameForScoreboard, currentTeam.team)
        }
    }
}
