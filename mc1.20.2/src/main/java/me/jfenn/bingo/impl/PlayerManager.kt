...
class PlayerHandle(
    ...
    override fun sendTeamMessage(message: IText, sender: IPlayerHandle, teamName: IText) {
        ...
        val decoratedMessage = parameters.type.chat.apply(message.value, parameters)
    ...
    override fun allHeldStackViews(): Sequence<IContainerItemView> {
        ...
        return sequence {
            ...
            ContainerItemView.Inventory(itemStackFactory.forStack(it))
        }
    }
    ...
    override fun giveOrEquipStack(stack: IItemStack) {
        ...
        if (slot != null && player.canEquip(itemStack) && slot != EquipmentSlot.MAINHAND && slot != EquipmentSlot.OFFHAND) {
    ...
    override fun setStack(slot: Int, stack: IItemStack) {
        if (slot in 0 until player.inventory.main.size) {
    ...
    override fun teleport(world: IServerWorld, pos: Vector3d, yaw: Float, pitch: Float) {
        player.teleport(world.world, pos.x, pos.y, pos.z, emptySet(), yaw, pitch)
    }
    ...
    override fun setSpawnPoint(
        ...
        player.setSpawnPoint(serverWorld.registryKey, spawn.toBlockPos(), angle, forced, sendMessage)
    }
