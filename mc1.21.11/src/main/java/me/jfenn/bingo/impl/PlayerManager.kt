...
class PlayerManager(
    ...
    override fun getOfflinePlayer(profile: PlayerProfile): IPlayerHandle {
        ...
        server.playerManager.loadPlayerData(player.playerConfigEntry)
        return PlayerHandle(player, itemStackFactory)
    }
...
class PlayerHandle(
    ...
    private fun getItemStackFactory() = itemStackFactory
        ?: BingoKoin.getScope(player.entityWorld.server)!!.get<IItemStackFactory>()

    override val world: IServerWorld
        get() = ServerWorldImpl(player.entityWorld)

    override val server: IMinecraftServer
        get() = MinecraftServerImpl(requireNotNull(player.entityWorld.server) { "Player $player entityWorld.server reference is null!" })
    ...
    override val commandSource: CommandSource
        get() = player.getCommandSource(player.entityWorld)
    ...
    override fun hasPermissionLevel(level: Int): Boolean {
        ...
        return player.permissions.hasPermission(permissions)
    }
    ...
    override val serverWorld: ServerWorld get() = player.entityWorld
    ...
    override fun setSpawnPoint(
        ...
        val respawnData = WorldProperties.SpawnPoint.create(
        ...
        player.setSpawnPoint(
            ServerPlayerEntity.Respawn(respawnData, forced),
            sendMessage
        )
    }

    override fun startRiding(entity: IEntity, force: Boolean) {
        player.startRiding(entity.entity, true, false)
    }
