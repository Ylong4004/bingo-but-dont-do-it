...
## 11. 2026-07-16 HUD、Tab 与同队去重复核

### 11.1 HUD 体验

- DDI 状态面板不再固定在右上角；在 Y 键界面的设置入口中新增“不要做挑战 HUD”，可拖拽位置、拖动边缘缩放，也可手动选择四角锚点及 X/Y 偏移。
- 新增独立的背景不透明度设置，默认 `30%`；文字与图标不会随背景一起变淡。
- 面板复用 Bingo 的禁止格图标、绿色边框和文本阴影；窄屏下会裁剪标题/正文并限制在屏幕内。
- 队伍共享模式中，其他队伍不再显示成员名单。每队改为两行：第一行只显示带队伍颜色的队名与剩余生命，第二行显示可见词条，避免长玩家名单挤掉词条。
- 打开 Tab、F1 隐藏 HUD、按配置隐藏 F3/聊天界面，以及打开 Bingo 自身界面时，DDI 面板不会遮挡对应界面。
- 该图形设置沿用 Bingo 现有 YACL 设置入口；未安装 YACL 时玩法本身仍可运行，但 Y 键界面不会提供这些高级配置控件。

### 11.2 Tab 剩余生命

- DDI 对局开始后创建临时 `LIST` 计分目标，Tab 中每名参赛玩家显示所属 objective 的权威剩余生命；共享模式下同队成员显示同一个数值，淘汰/离队成员显示 `0`。
- 若 `LIST` 已被其他模组或管理员计分板占用，DDI 不覆盖它，而是在玩家名后追加 `N♥`；对局中每秒复核槽位，发生接管时自动切换。
- DDI 只删除自己本局创建的计分目标；同名外部目标也不会被复用、改写或清理。
- 对局结束后撤销名称后缀、清除自己的显示槽并刷新玩家列表；与准备状态的 Ready 标记按顺序组合。

### 11.3 同队已触发词条不再抽取

- 每局按 `BingoTeamKey` 保存硬历史；个人模式下同一 Bingo 队的多个个人 objective 也共享这份历史，其他队伍互不影响。
- 普通扣血词条以及“立即扣心/立即回血”都会在结算时写入历史；仅因倒计时自然到期而换掉的未触发词条不会写入。
- 去重使用稳定 `repeatKey`，默认等于 trigger 类型。因此“发送聊天消息/打字说话”这种文案不同但实际规则相同的条目，只会在同队触发一次。
- 抽取永不放宽已经触发的硬历史；同队当前正在使用的规则作为软排除，只有候选不足时才允许并行重复。
- 某队真正触发完所有可用 `repeatKey` 后，明确按完成 DDI 词池结算，不会偷偷重新抽到旧规则。
...
- `:common:test :integration-ddi:test` 合计 40 个测试，0 failure、0 error、0 skipped。
- `:mc1.21.11:build` 正式 remap 构建成功。
- 最终 JAR 共 4,340 个 ZIP 条目、0 个重复路径；HUD 放置页、Tab 生命服务、队伍历史和名称装饰器等新增 class 均存在。
