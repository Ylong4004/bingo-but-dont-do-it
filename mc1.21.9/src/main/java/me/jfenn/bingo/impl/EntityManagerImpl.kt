...
class EntityManagerImpl(
    ...
    private fun forEntity(entity: Entity): IEntity {
        val impl = when (entity) {
            ...
            is TextDisplayEntity -> TextDisplayEntityImpl(entity, textFactory)
    ...
    override fun <T : IEntity> createEntity(type: EntityType<T>, world: ServerWorld): T {
        ...
        val entity = mcType.create(world, SpawnReason.STRUCTURE)!!
...
private class NbtBooleanDelegate(private val name: String) {
    operator fun getValue(thisRef: EntityImpl, property: KProperty<*>): Boolean = thisRef.nbt.getBoolean(name, false)
...
private class NbtIntDelegate(private val name: String) {
    operator fun getValue(thisRef: EntityImpl, property: KProperty<*>): Int = thisRef.nbt.getInt(name, 0)
...
private class NbtFloatDelegate(private val name: String) {
    operator fun getValue(thisRef: EntityImpl, property: KProperty<*>): Float = thisRef.nbt.getFloat(name, 0f)
...
open class EntityImpl(
    ...
    override var pos: Vector3d
        get() = entity.entityPos.let { Vector3d(it.x, it.y, it.z) }
        set(value) {
    ...
    override var commandTags: Set<String>
        ...
        set(value) {
            entity.commandTags.toSet().forEach { entity.removeCommandTag(it) }
    ...
    internal var nbt
        get() = NbtWriteView.create(BingoErrorReporter.getInstance(), entity.registryManager)
            .also { entity.writeData(it) }
            .nbt
        set(value) {
            entity.readData(NbtReadView.create(BingoErrorReporter.getInstance(), entity.registryManager, value))
        }
...
class TextDisplayEntityImpl(
    ...
    override var value: IText
        get() = textFactory.from(entity.text)
        set(value) {
    ...
    override var billboard: ITextDisplayEntity.Billboard
        get() = nbt.getString("billboard")
            ...
            ?: ITextDisplayEntity.Billboard.FIXED
        set(value) {
    ...
    override var alignment: ITextDisplayEntity.TextAlignment
        get() = nbt.getString("alignment")
            ...
            ?: ITextDisplayEntity.TextAlignment.CENTER
        set(value) {
...
class BlockDisplayEntityImpl(
    ...
    override var blockIdentifier: String
        get() = Registries.BLOCK.getId(entity.blockState.block).toString()
        set(value) {
            val block = Registries.BLOCK.get(Identifier.of(value))
            entity.blockState = block.defaultState
        }
}
