...
class ServerEventsImpl(
    ...
    init {
        ...
        UseBlockCallback.EVENT.register { player, world, hand, hit ->
            ...
            val playerImpl = getPlayerImpl(player)
            val results = eventBus.emit(UseBlockEvent, UseBlockEvent(world.server, playerImpl, world, hand, hit))
        ...
        UseEntityCallback.EVENT.register { player, world, hand, entity, hit ->
            if (player !is ServerPlayerEntity || world !is ServerWorld) return@register net.minecraft.util.ActionResult.PASS
            val playerImpl = getPlayerImpl(player)
            val results = eventBus.emit(UseEntityEvent, UseEntityEvent(world.server, playerImpl, world, hand, entity, hit))
            ActionResult.collapse(results)
                ?.toActionResult()
                ?: net.minecraft.util.ActionResult.PASS
        }
        ...
        UseItemCallback.EVENT.register { player, world, hand ->
            if (player !is ServerPlayerEntity || world !is ServerWorld) return@register net.minecraft.util.ActionResult.PASS
            val playerImpl = getPlayerImpl(player)
            val results = eventBus.emit(UseItemEvent, UseItemEvent(world.server, playerImpl, world, hand))
            ActionResult.collapse(results)
                ?.toTypedActionResult()
                ?: net.minecraft.util.ActionResult.PASS
        }
        ...
        AttackEntityCallback.EVENT.register { player, world, hand, entity, hit ->
            if (player !is ServerPlayerEntity || world !is ServerWorld) return@register net.minecraft.util.ActionResult.PASS
            val playerImpl = getPlayerImpl(player)
            val results = eventBus.emit(AttackEntityEvent, AttackEntityEvent(world.server, playerImpl, world, hand, entity, hit))
            ActionResult.collapse(results)
                ?.toActionResult()
                ?: net.minecraft.util.ActionResult.PASS
        }
