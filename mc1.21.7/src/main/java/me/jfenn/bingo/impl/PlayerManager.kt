...
class PlayerManager(
    ...
    override fun getOfflinePlayer(profile: PlayerProfile): IPlayerHandle {
        ...
        server.playerManager.loadPlayerData(player, BingoErrorReporter.getInstance())
        return PlayerHandle(player, itemStackFactory)
    }
...
class PlayerHandle(
    ...
    override val world: IServerWorld
        get() = ServerWorldImpl(player.world)
    ...
    override val commandSource: CommandSource
        get() = player.getCommandSource(player.world)
    ...
    override val serverWorld: ServerWorld by player::world
