...
class FramebufferImpl(
    ...
    override fun write(callback: (IDrawService) -> Unit) {
        ...
        RenderSystem.getModelViewStack().translate(0f, 0f, -11000.0f)
    ...
    override fun draw(service: IDrawService, width: Int, height: Int) {
        ...
        bufferBuilder.vertex(matrix4f, x1, y1, 0.0f).texture(u1, v1).next()
        bufferBuilder.vertex(matrix4f, x1, y2, 0.0f).texture(u1, v2).next()
        bufferBuilder.vertex(matrix4f, x2, y2, 0.0f).texture(u2, v2).next()
        bufferBuilder.vertex(matrix4f, x2, y1, 0.0f).texture(u2, v1).next()
